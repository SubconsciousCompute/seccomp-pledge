...
pub fn with_no_restrictions() {
    ...
    cmd.args(
        ["-no-check", "-v", ".", "-p", "stdio", "-p", "rpath", "-p", "tty", "ls"])
        .assert()
...
pub fn with_unveil_restrictions() {
    ...
    // Escapes quotes, so promises are supplied separately
    cmd.args(
        ["-no-check", "-p", "stdio", "-p", "rpath", "-p", "tty", "ls"])
        .assert()
...
pub fn criterion_benchmark(c: &mut Criterion) {
    c.bench_function("add_bench", |b| {
        b.iter(|| black_box(with_no_restrictions()))
    });
    c.bench_function("add_bench", |b| {
        b.iter(|| black_box(with_unveil_restrictions()))
    });
    c.bench_function("add_bench", |b| {
        b.iter(|| black_box(with_pledge_restrictions()))
    });
}
